...
>* BlockMapping内的东西确实是不落地的;可以通过Namespace中的内容以及ChunkServer的block report重建出来。
>* 每次NameServer重启时都会重建Blockmapping中的内容。
>* 耗时主要是顺序扫描leveldb的过程。
...
>* Sdk在向cs发起读请求时，给cs提供的参数是需要进行读取的block id。比如说现在要读A文件，然后其它人将A rename为了B，但是A中block的id是不变的，所以可以正常读。
>* 如果要读A时，B被rename成了A，那么A之前的block会被gc策略删除，但是即使物理文件被删除，在fd尚未被close之前，仍然可以读到A中的数据。*这也与linux中的语义是一样的（linux中也可以删除正在读写的文件，只有当该文件的最后一个fd被关闭时，文件才真正的被删除）*
...
9.	AddBlock在什么时候会被调用？
> AddBlock在SDK首次向一个新创建的文件调用Write时被调用，然后sdk会与NameServer通信，Namserver为其分配block id，以及挑选出足够的ChunkServer供sdk写入使用。
...
12.	链式写有什么问题么？
>* 链式写有可能存在慢节点，一旦链中某个结点速度较慢，则会拖慢整个链的写入速度
>* 现在也支持扇出写，同时写四个副本，只要有三个成功就返回成功，可以去规避慢节点。
...
>* 每次sdk向ChunkServer发起写请求后，若sdk收到ChunkServer返回的写成功，则会移动自己维护的滑动窗口，当滑动窗口编号的上限小于sdk当前将要发送的写请求编号时，会将当前的写请求Dealy（Delay的是线程池中的task）
...
by 孙金金
---
1. chunkserver的version是哪儿来的？
>* chunkserver的namepace_version要和nameserver保持一致，cs向ns发送心跳时会把自己保存的version发给ns；
>* 当cs的version与ns不一致时，cs会重新向ns register，将namespace_version重置

2. version是什么概念？为什么会不一致？
>version有两种：namespace_version和block version；
>* namespace_verison记录namespace的版本，在刚开始启动时，如果为空，会根据当前时间置version；当nameserver重启时，有可能对verison进行重置，从而chunkserver中保存的version与namespace不一致；
>* block_version是在写数据时，buffer的seq；可以根据block_version对数据不一致的情况，进行检查和恢复；

3. 有哪几种version？
>同2

4. AddBlock的时候已经记录了block所在的cs位置，为什么还要report？
>* 首先，只有chunkserver才对其上面存储的block有最终发言权；
>* 其次，在上报的过程中可以发现obsolete_block(可以发起删除), 需要增加replicas的block(可以向其他chunk server发起PushBlock，恢复副本)，以及需要close的block(可以保证数据的一致性)

5. report的check都有哪些？
>* chunk server端的namespace_version和nameserver是否一致，不一致时需要对version进行重置；
>* 以及4中提到的对block进行删除、增加副本数、close的情况；

6. 删除一个文件的标志是什么，需要删除物理文件？
> 文件的元数据信息删除成功即成功，物理文件后续根据report进行删除

7. 操作目录树的时候需要锁么？
>目前是没有锁的，但是需要加锁；
比如：对一个文件操作时，可能会有可能 sdk删除请求成功后，chunkserver才上报完成 文件的写 的情况。

8. Chunkserver接收到写请求的时候，什么时候往磁盘写？
>chunkserver维护了一个recv_window，当队列中的数据有序时，就会往磁盘写

9. SDK写的时候什么时候算写成功？
>* 当链式写时，只有都写成功才会返回成功；
>* 当扇出写时，至少两个写成功，就返回成功

10. SDK写的时候有一个cs宕机，怎么处理？
> 当cs宕机时，会把文件关闭，标志incompelete，后面进行副本恢复

11. GFS写的时候有一个主副本，为什么？
> 主要是GFS运行多个client对同一个文件写，会出现顺序不一致，有主副本来维持数据的一致性，降低master的负载

12. 链式写和扇出写应用场景有什么差别？
>* 链式写的延迟较高，一个节点慢就会把整个链条拖慢；
>* 扇出写可以规避慢节点

13. 写的时候有一台cs挂机，还能继续写成功么？
> 写的时候如果一台cs挂机，不能继续写；close文件，落盘

14. cs内存满和挂机，在写的时候处理一样么？
>不一样
>* 如果内存满，会返回kCsTooMuchUnfinishedWrite，从而重试；
>* 如果挂机不会重试。

15. 什么时候会发起副本恢复？
> 两种情况：
>* 一个是block report时，nameserver会检查副本数发起恢复；
>* 一个是chunkserver宕机时，nameserver会把当前chunkserver上所有的block副本恢复。

16. report的时候report哪些内容？report多少block？
>* block [block_id,version,block_sieze,is_recover], cs_addr, cs_id等 ；
>* 一次report 会report一组block，在第一轮report和后面block数目不同；但最终所有block都会report

17. block的位置信息为什么不全部记下来？为什么要靠report？
>* 首先，block的信息记下来没有意义；
>* 其次，记录的代价比较高；当机器连续挂机重启时，就需要不断的读、删、写磁盘。

18. cs挂机，重启后磁盘上数据全丢，ns怎么知道的？
>* chunkserver重启时会从disk上load一遍磁盘上的所有block；
>* 在report的时候，namesever就能发现

19. cs掉盘的时候，怎么处理的？(meta还在，但实际物理文件丢失)
>同上
